...
namespace n01629177Cumulative1.Controllers
{
  public class CourseDataController : ApiController
  {
    ...
    [HttpGet]
    public IEnumerable<Course> ListCourses(int by_teacher_id = -1)
    {
      ...
      if(by_teacher_id > 0){
        //A `teacher_id` has been specified, return the list of classes that has the specified teacher.
        command.CommandText = "select * from classes where teacherid = @by_teacher_id;";
        command.Parameters.AddWithValue("@by_teacher_id", by_teacher_id);
        command.Prepare();
      }
      else {
        //Otherwise, just return all the classes.
